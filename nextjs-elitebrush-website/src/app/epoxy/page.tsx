...
import GallerySection from "@/app/components/GallerySection"; // Reuse the same gallery component

// Updated query to match painting page structure with mainImages array
const EPOXY_QUERY = `*[
  _type == "epoxy"
...
export default async function EpoxyPage() {
  ...
  // Filter to get epoxy projects with main images for portfolio display
  const portfolioEpoxy = epoxyProjects.filter(
    project => project.mainImages && Array.isArray(project.mainImages) && project.mainImages.length > 0
  );
  ...
  return (
    <main className="relative min-h-screen bg-gradient-to-b from-black/90 to-red-500">
      ...
      <section className="relative z-10 pt-32 pb-12">
        <div className="container mx-auto px-6 md:px-10">
          <div className="max-w-7xl mx-auto w-full">
            ...
            <h1 className="text-4xl font-bold text-white mb-4">Our Epoxy Work</h1>
            <div className="w-20 h-1 bg-white mb-6"></div>
            <p className="text-xl text-white/80 max-w-3xl mb-10">
              Browse our transformative epoxy services and see the quality of our craftsmanship.
            </p>
      ...
      {/* Use the Gallery Component with epoxy projects */}
      <GallerySection portfolioPaintings={portfolioEpoxy} />
          ...
          {projectsWithSliders.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {projectsWithSliders.map((project) => (
                <div key={project._id} className="transform transition duration-300">
                  <h3 className="text-xl font-medium text-white mb-3 pl-1">{project.title || "Transformation"}</h3>
                  <BeforeAfterSlider
